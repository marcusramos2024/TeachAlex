...
interface GraphProps {
  ...
  updateNodePositions: (nodes: SubConcept[]) => void;
}
...
const Graph = ({ nodes, currentConceptIndex, updateNodePositions }: GraphProps) => {
  ...
  useEffect(() => {
    ...
    nodes.forEach((node, index) => {
      ...
      const jitter = 0.03; // Reduced from 0.05 for less chaos
    ...
    let resolvedPositions = newPositions;
    
    // Apply multiple iterations of overlap resolution
    for (let i = 0; i < 5; i++) {
      resolvedPositions = resolveOverlap(
        nodeIds, 
        resolvedPositions, 
